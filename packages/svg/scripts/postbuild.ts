/**
 * Postbuild for @modelicons/svg:
 *   1. Add .js/.cjs extensions to relative imports in dist/ so Node ESM works
 *   2. Inject per-brand subpath exports into package.json
 */
...
const resolveTarget = (fromFile: string, spec: string, ext: '.js' | '.cjs'): string | null => {
  ...
  // .cjs file referencing a .js → rewrite to .cjs (tsup emits .js extensions
  // even in CJS bundles because it doesn't know about our outExtension rule)
